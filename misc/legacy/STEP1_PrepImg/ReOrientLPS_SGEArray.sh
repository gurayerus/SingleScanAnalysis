...
exedir='/cbica/projects/ADNI/Pipelines/ADNI_DLICV_2022/Scripts/STEP1_PrepImg'
scr=${exedir}/ReOrientLPS.sh
...
if [ -e $inImg ]; then
    ${scr} $inImg ${outDir}/${SUB} > ${outDir}/${SUB}/sgelogs/ReOrientLPS.sh-${JOB_ID}-${SGE_TASK_ID}.log 2>&1;
fi
