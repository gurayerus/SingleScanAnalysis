...
exedir="${pdir}/Scripts/STEP6_FSTIV"
scr=${exedir}/ApplySAMSEG.sh
...
## Make output folder
mkdir -pv ${outDir}/sgelogs

## Run script
inImg=${imgDir}/${SUB}/${SUB}_T1_LPS.nii.gz

${scr} $SUB $inImg ${outDir} > ${outDir}/sgelogs/ApplySAMSEG.sh-${JOB_ID}-${SGE_TASK_ID}.log 2>&1;
