...
exedir='/cbica/projects/OASIS/OASIS3/Pipelines/OASIS3_DLICV_2022/Scripts/STEP2_DLICV'
...
## Run script
inSub=${inDir}/${SUB}
inImg=${inSub}/${SUB}_T1_LPS.nii.gz
outSub=${outDir}/${SUB}
mkdir -pv $outSub
outImg=${outSub}/${SUB}_T1_LPS_dlicvmask.nii.gz

if [ -e $inImg ] && [ ! -e $outImg ]; then
     ${scr} $inSub $inImg $outSub $outImg > ${outSub}/sgelogs/ApplyDLICV_SingleImg_cpu.sh-${JOB_ID}-${SGE_TASK_ID}.log 2>&1;
# echo  ${scr} $inSub $inImg $outSub $outImg
fi
